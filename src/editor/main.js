...
// --- MONSTER BUILDER MODAL (World Editor roadmap section E) ---
// A reusable monster TYPE catalog, authored via the first tabbed-modal UI
// in this codebase (src/editor/modal.js). Launched from Monsters mode.
// Slot shape-editing mirrors Object Builder mode's mechanics (palette,
// numeric transform fields, duplicate/delete) but operates on an author-
// defined body-part slot's shapes instead of a flat top-level list, and
// renders inside its own small isolated Scene/Camera/Renderer (mirroring
// src/character-creation/main.js's pattern) rather than the shared big
// editor canvas, since the modal overlays it.
...
let monsterTypeCatalogById = {}; // kept in sync with monsterTypeCatalog — threaded into generateMonster() so Monsters-mode placement markers render catalog types correctly instead of falling back to generateSlime
...
let mbRig = {}; // role -> pivot Group, rebuilt whenever the modal loads/clears a monster type
...
fetch('/api/monster-types')
  ...
  .then((list) => {
    ...
    refreshMbCatalogList(); // also rebuilds monsterTypeCatalogById
...
// --- Mini isolated 3D workspace (own Scene/Camera/Renderer, not the shared editor canvas) ---
...
// selector specificity, permanently pinning the canvas's layout box to
// whatever window.innerWidth/innerHeight were at page-load time, no matter
// how the modal/pane actually gets sized afterward. Clearing it here hands
// layout sizing back to CSS entirely; resizeMbCanvas() below always calls
// setSize with updateStyle=false so this stays cleared.
mbCanvas.style.width = '';
mbCanvas.style.height = '';
const mbScene = createScene();
// Starting position only — frameMbCamera() (below) recomputes position/
// target from the actual body's bounding box on every rebuild, since the
// preview pane's aspect ratio varies (responsive width, see editor.html)
// and a fixed camera framed for one aspect cuts off content at another.
...
function animateMbWorkspace() {
  ...
  if (!monsterBuilderModal.isOpen()) return;
...
// Started at the bottom of this section (see `animateMbWorkspace();` near
// the modal wiring below) — it references `monsterBuilderModal`, a const
// declared later in this same module; calling it here would throw (TDZ)
// since this runs synchronously during module evaluation, not deferred.
 ...
 * pane is a tall, narrow portrait rectangle (fixed width, stretched to the
 * dialog's full height), and a fixed camera framed for one aspect ratio
 * cuts off content at another. Same Box3-based approach already used for
 * thumbnail rendering (see renderThumbnail below), applied live here.
 */
...
/** Rebuild the mini workspace from scratch — used on New/Load, not on every shape edit. */
...
// --- Preset thumbnails: a small dedicated offscreen renderer, not the shared editor canvas or the mini workspace ---
...
function applyBodyPreset(presetId) {
  ...
  monsterBuilderModal.showTab('model');
}
...
// Object Builder's shape list above.
...
mbCanvas.addEventListener('pointerdown', (e) => {
  const rect = mbCanvas.getBoundingClientRect();
...
// Same add/remove-a-row shape as Monsters mode's per-placement loot list, but
// this one lives on the catalog TYPE, so it applies to every spawn of that
// type — including the ones already scattered across a saved map, which
...
function loadMonsterTypeIntoModal(mt) {
  refreshLootQuestOptions('mb-loot-quest'); // a quest authored since the last open
...
const monsterBuilderModal = createTabbedModal('monster-builder-modal', [
...
document.getElementById('mb-close-btn').addEventListener('click', () => monsterBuilderModal.close());
document.getElementById('open-monster-builder-btn').addEventListener('click', () => {
  loadMonsterTypeIntoModal(null);
  monsterBuilderModal.open();
  setTimeout(resizeMbCanvas, 0); // canvas has no size until the modal's display:flex takes effect
});

animateMbWorkspace(); // safe here — monsterBuilderModal is declared above this point
...
// Monster Builder modal if it's open, the Object Builder if that's the
// current mode, otherwise the map itself (see undoLastAction). Skipped while
// a form field has focus so the browser's own native per-field undo (e.g. an
// in-progress number-input edit) takes precedence instead of fighting with
// this. No redo in this pass.
window.addEventListener('keydown', (e) => {
  if (!(e.ctrlKey || e.metaKey) || e.key.toLowerCase() !== 'z') return;
  if (isTypingInFormField()) return;
  const mbOpen = document.getElementById('monster-builder-modal').style.display !== 'none';
  e.preventDefault();
  if (mbOpen) undoMb();
  else if (mode === 'object-builder') undoBuilder();
  else undoLastAction();
});
